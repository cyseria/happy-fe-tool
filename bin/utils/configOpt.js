...
module.exports = {
     ...
     */
    async getConfigFilePath(rule, tplName, supportFile) {
        ...
        const existTpl = Object.keys(tpls).filter(tpl => {
            if (
                Object.prototype.hasOwnProperty.call(tpls[tpl], rule.name)
                && supportFile.includes(tpls[tpl][rule.name])
            ) {
     ...
     */
    getConfigTargetPath(sourcePath) {
